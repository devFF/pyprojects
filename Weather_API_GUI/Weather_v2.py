...
class App:
    def __init__(self):
        ...
        self.greetings = tk.Label(self.window, text='Select city:', font=('Times New Roman', 20), justify='left', width=9)
        ...
        if self.city_mes == 'no':
            messagebox.showinfo(title='Location', message='Choose a city from the list')
    ...
    def weather_info(self, event = None):
        ...
        weather_data = tk.Label(
            self.window,
            text='Now temperature in {} is {} C, feels like {} C'.format(self.city,self.temperature,self.real_temperature),
